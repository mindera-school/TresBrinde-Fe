import { BUDGET_CREATE_REQUEST, BUDGET_CREATE_SUCCESS, BUDGET_CREATE_FAIL, BUDGET_LIST_REQUEST, BUDGET_LIST_SUCCESS, BUDGET_LIST_FAIL } from './../../constants/constants';
import { CreateBudgetActionTypes, IBudget, ListBudgetActionTypes } from './../types/IBudget';
...
> = (state = initialBudgetList, action: ListBudgetActionTypes) => {
  switch (action.type) {
    ...
    case BUDGET_LIST_SUCCESS:
      console.log(Object.entries(action.payload)[0].slice(1) as IBudget[]);
      return {
        loading: false,
